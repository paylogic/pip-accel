...
class PipAccelerator(object):
    ...
    def install_from_arguments(self, arguments, **kw):
        ...
        """
        requirements = self.get_requirements(arguments, use_wheels='--no-use-wheel' not in arguments)
        if self.setuptools_upgrade_needed(requirements):
    ...
    def setuptools_upgrade_needed(self, requirements):
        """
        Check whether setuptools should be upgraded for wheel support.

        If the requirement set includes wheel distributions then setuptools 0.8
        or later needs to be installed in order for the wheel installations to
        function properly. This method includes ``setuptools >= 0.8`` in the
        requirement set when needed.

        :param requirements: A requirement set generated by
                             :py:func:`get_requirements()`.
        """
    ...
    def unpack_source_dists(self, arguments, use_wheels=False):
        ...
        """
        unpack_timer = Timer()
        logger.info("Unpacking source / wheel distribution(s) ..")
        with PatchedAttribute(pip_install_module, 'PackageFinder', CustomPackageFinder):
            ...
            logger.info("Finished unpacking %s in %s.",
                        pluralize(len(requirements), "source / wheel distribution"),
                        unpack_timer)
            return requirements
    ...
    def install_requirements(self, requirements, **kw):
        ...
        for requirement in requirements:
            if requirement.is_installed:
                ...
                # versions around.
                self.uninstall_package(requirement.name)
                ...
                if requirement.name == 'setuptools':
                    self.uninstall_package('distribute')
    ...
    def uninstall_package(self, pkg_name):
        """
        Use ``pip`` to remove an installed package.

        :param pkg_name: The name of the package to remove (a string).
        """
        if run('{pip} uninstall --yes {package} 1>/dev/null 2>&1',
                pip=self.pip_executable,
                package=pkg_name):
            logger.info("Uninstalled previously installed package %s.", pkg_name)
